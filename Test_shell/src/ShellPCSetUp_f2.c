...
/* ------------------------------------------------------------------- */
/*
   ShellPCSetUp_f2 - This routine sets up a user-defined
...
/*  -----> uncomment if you choose  H = diag(H)   */
/*----------------------------------------------------------------------*/
  // Vec            diag;
...
/*----------------------------------------------------------------------*/
  ...
  MatTransposeMatMult(user->subA[1],user->subA[1],MAT_INITIAL_MATRIX, 2.6,&E);
  /*  use D intead user->subA[0] of if you choose H = diag(H) ! Don't forget to uncomment the code above */ 
  MatAYPX(E,gamma,user->subA[0],DIFFERENT_NONZERO_PATTERN);
